...
def get_file_changed_name(file_path: str, name: str) -> str:
    """
    Changes the name of the given file, keeping the extension.
    :param str file_path: the path to the file.
    :param str name: the name of the new file (without extension).
    :rtype: str
    :return: the file path with the new name.
    """
    return os.path.join(os.path.dirname(file_path),
                        f'{name}.{get_file_extension(file_path)}')


def get_file_changed_extension(file_path: str, ext: str, prefix: str = '', suffix: str = '') -> str:
    """
    Changes the extension of the given file, keeping the original name.
    :param str file_path: the path to the file.
...
def load_dict_json(file_path: str) -> Dict[str, Any]:
    """
    Loads a dictionary structure from a JSON file.
    :param str file_path: the path to the JSON file from which to load the dictionary.
    :rtype: dict
    :return: a dictionary structure loaded from the JSON file.
    """
    with open(file_path, 'r') as fp:
        return json.load(fp)


def save_object(obj, file_path: str, compress_gzip: bool = True, asynchronous: bool = False) -> Optional[Thread]:
    """
    ...
    :param bool asynchronous: whether to write the object in the file asynchronously in a new thread.
    :rtype: Thread or None
    :return: if `asynchronous=True`, the thread used to write the object to file, otherwise `None`.
    """
    ...
    if asynchronous:
        ...
        thread.start()
        return thread  # will need to join the thread later on

    _write()  # execute the write operation synchronously
