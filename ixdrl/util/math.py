...
def running_average(arrays: List[np.ndarray],
                    axis: Optional[int] = None) -> np.ndarray:
    """
    Computes the mean of a list of arrays using a running average algorithm.
    Useful for when the arrays and/or list are very large and we just need the mean (less memory consumption).
    :param list[np.ndarray] arrays: the list of arrays to be averaged. The arrays should have the same shape on
    axes != `axis` such that the arrays can be averaged together.
    :param int axis: the axis (axes) over which the mean is to be computed.
    :rtype: np.ndarray
    :return: the mean array of the same shape as input arrays except on axes != `axis`, or a single value if
    `axis==None`.
    """
    assert len(arrays) > 0, 'Empty list given'

    mean: np.ndarray = None
    count: int = 0
    for a in arrays:
        _count = a.shape[axis] if axis is not None else np.prod(a.shape)
        _mean = np.mean(a, axis=axis)
        if mean is None:
            mean = _mean
            count = _count
        else:
            total = count + _count
            mean = (count / total) * mean + (_count / total) * _mean
            count = total
...
def get_ravel_values(a: np.ndarray, slices: Tuple[Union[slice, int, None], ...], shape: Tuple[int]) -> np.ndarray:
    """
    Gets the values of a flat array as if the array was multi-dimensional.
    :param np.ndarray a: the array whose values we want to access.
    :param tuple[slice] slices: the slices indicating the indices of the multi-dimensional array.
    :param tuple[int] shape: the shape of the multi-dimensional array.
    :rtype: np.ndarray
    :return: a flat array with the retrieved values.
    """
    # checks inputs
    a = np.asarray(a)
    if len(a.shape) > 1:
        raise ValueError(f'Expected flat array, but array with shape: {a.shape} given')
    if a.size != np.product(shape):
        raise ValueError(f'Shape: {shape} results in a size different than the provided array\'s: {a.size}')

    if len(slices) != len(shape):
        raise ValueError(f'Trying to get values for array with shape {shape}, '
                         f'but only {len(slices)} slices provided.')

    # select the indices from the multi-dimension slices
    slice_idxs: List[slice] = []
    for i, s in enumerate(slices):
        if s is None:
            slice_idxs.append(slice(0, shape[i]))  # full range slice
        elif isinstance(s, int):
            slice_idxs.append(slice(s, s + 1))  # int index to slice
            ...
            raise ValueError(f'Unknown slice type: {s}, only slice or int permitted')

    slice_idxs = np.mgrid[slice_idxs].reshape(len(shape), -1)  # use mgrid to find combinations of (multi-dim) indices
    idxs = np.ravel_multi_index(slice_idxs, shape)  # ravel to get the flat indices
    return a[idxs]


def one_hot_encode(a: np.ndarray, nb_classes: int) -> np.ndarray:
    """
    One hot encodes the given array to a specified number of classes.
    See: https://stackoverflow.com/a/42874726/16031961
    :param nd.array a: the array whose elements we want to one hot encode.
    :param int nb_classes: the number of classes
    :rtype: np.ndarray
    :return: an array containing 0s and 1s of shape `(*a.shape, nb_classes)`.
    """
    a = np.asarray(a)
    if np.any(a >= nb_classes):
        raise ValueError(f'Some of the provided values {a} exceed the given number of classes {nb_classes}')

    res = np.eye(nb_classes)[a.reshape(-1)]
    return res.reshape(list(a.shape) + [nb_classes]).astype(np.uint8)
